...
Customized Frappe Docker deployment for a German ERPNext installation. Based on the official [frappe_docker](https://github.com/frappe/frappe_docker) repository, extended with Germany-specific apps (DATEV, ERPNext Germany, EU eInvoice) from ALYF. Images are built and pushed to GitHub Container Registry (ghcr.io) via GitHub Actions on the `release` branch.
